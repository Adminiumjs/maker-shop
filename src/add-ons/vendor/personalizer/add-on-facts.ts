/*
 * VENDORED from add-ons/packages/personalizer/src/add-on-facts.ts — synced by scripts/sync-add-ons.sh.
...
export const INERT_ORIGINS: readonly { origin: string; why: string }[] = [
  {
    ...
    why: "the SVG XML namespace, written into every picture this add-on draws so the markup declares which vocabulary it is in. An XML namespace is an identifier; no agent dereferences one",
  },
];
