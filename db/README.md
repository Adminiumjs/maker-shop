...
`docker compose up` gives you a working product: `studio-db` creates the
...
By default these talk to the `studio-db` container through
...
```bash
DATABASE_URL=postgres://user:pass@host:5432/birchrow npm run demo:status
```
